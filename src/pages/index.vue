<template>
 ...
 <!-- 繞著愛心路徑的文字 -->
          ...
          <v-col v-for="(item, index) in thirdColItems" :key="index" cols="12">
            <v-sheet
              class="d-flex align-center justify-center sheet"
              :color="item.type === 'color' ? item.value : ''"
              height="600" style="position: relative; overflow: hidden;"
            >
              <template v-if="item.type === 'image'">
                <v-img :src="item.value" height="100%" cover></v-img>
              </template>
 ...
 <!-- 繞著愛心路徑的文字 -->
      ...
      <v-col cols="12" md="4" class="d-flex ma-n3">
        <v-col cols="12">
          <v-sheet class="d-flex gradient-sheet"  height="774" :style="{ background: randomGradient,position: 'relative', overflow: 'hidden' }">
    ...
    <svg viewBox="0 0 120 120">
      ...
      <path id="heartPath2"
          ...
          <!-- 彈出 "Oshipalette" 文字 -->
      <sheet-foooter id="popupText" class="popup-text" :style="logoColor" >Oshi Palette</sheet-foooter>
  ...
  <v-container :style="useStyle.homeNewsBg" class="py-10 "  fluid>
    <h1 class="text-center pt-10">最新活動消息</h1>
        ...
        <v-timeline-item v-for="event in latestEvents" :key="event._id" :size="$vuetify.display.smAndDown ? 'small' : 'large'">
          <template #icon>
            <v-avatar>
          ...
          <template #opposite v-if="$vuetify.display.mdAndUp" >
